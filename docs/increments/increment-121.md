...
- 利用者の判断により、残る応答shape検査は通常利用で問題が出た際に確認する。実装とprobeをcommit `aff155b5`で`origin/main`へpushし、同じclean commitからDeno 2.9.7でbuildした`henji 0.5.0`（build `a32d1ad3…`、SHA-256 `dbac779c…`）を`~/.local/bin/henji`へ原子的に配置した。配置先の`--version`とbuildしたbinaryとのhash一致を確認した。
