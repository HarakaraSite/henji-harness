...
- Execution IDが得られないsettle経路（artifact永続化失敗等）では、従来どおりIDなしの案内
  （`/recall without an ID references the latest stopped execution's …`）を表示する。IDを捏造しない。
...
- turn停止時、Host `ExecutionCoordinator.settleExecution`／`persistExecutionArtifact`が実行をnon-canonicalに
  settleし、`turnEndFromOutcome`が`turn_end` AgentEventを発行する。このeventは`executionArtifactId`
  （`execution.executionId`＝`/recall`の`prepareRecall`が照合するID）を既に持つ。
...
- `v0/presentation/adapter_projection.ts`: `outcome()`が`LoopOutcome.executionArtifactId`を`executionId`へ
  投影する。
- `v0/presentation/tui_presentation_adapter.ts`: `turn_end`→`failure_diagnostic`発行で
  `event.executionArtifactId`を`executionId`として渡す。
