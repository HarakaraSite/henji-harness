...
状態: 2026-09-27、利用者が「入り口だけ作り、キーの保管場所は現状維持」という方針で、
次のincrementとして詳しい計画の作成を依頼した。本文書はHenjiへ渡す実装計画であり、
productコードの実装、実provider call、配置・公開はまだ指示されていない。
command名、画面操作、内部interfaceは以下の計画案とする。
