...
構想上、通常利用で観測された困難や有用な成功は、原因を考え、Henjiの変更候補を作り、次の利用で効果を
確かめるための材料になる。この関係は自己改訂の性質を説明するものであり、現在の開発段階や固定された
実施手順を定めるものではない。
...
観測、候補生成、評価、適用のどこまでを自動化するか、人間がどの判断を担うかはこの構想では決めない。
...
着手時期、前提機能、実装順序、比較方法、Human Gateは未決定である。
