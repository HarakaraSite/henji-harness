...
- 現在のproduction利用方法: [`../README.md`](../README.md)
- Workerの実装・受入結果:
...
| --- | --- | --- | --- |
| F01 | 人間がHenjiへ依頼し、進捗を見て、結果を受け取り、通常利用を続けられる | 改訂前後の経験を生む通常利用。SurfaceはHostが所有する | **部分実装**。production TUIと非対話commandがある。Worker real-provider受入は完了。integrated TUIは条件付き合格で、最終daily-use adoptionは未記録 |
| F02 | provider/model、複数step、tool call/result、planner delegationを含むagent turn | AgentCompositionとturn semanticsはWorkerが所有する | **実装済み・受入済み**。`loop.ts`、`worker_runtime.ts`、`worker_agent_api.ts`とreal-provider受入結果に証拠がある |
...
| F04 | Sessionのtranscriptとcontext checkpointを再起動後も利用する | Hostがstorageを所有し、Workerがconversation/contextの意味を所有する | **実装済み・受入済み**。workspace-partitioned session schema-v1/v2、atomic commit、reopen、automatic compactionがある |
| F05 | 人間が保存済みhistoryとcontextを通常利用中に参照する | 経験を人間が確認し、後の改訂指示へ使うSurface機能 | **部分実装**。session pickerはTUIから利用可能。history/context projectionとcontroller内部処理はあるが、現TUIのslash entry pointはない |

### Definition、Host / Worker、durability

| ID | 必要な機能 | architecture上の責務・境界 | 現コードの状態 |
| --- | --- | --- | --- |
| F06 | executable TypeScriptのAgentDefinitionがprovider/model、loop、tools、subagents、contextをAgentCompositionへ合成する | Definition評価とcomposition実行はWorker | **実装済み・受入済み**。built-in `default` / `planner`とworkspace-local external Definitionがある。現factoryが表現する項目だけが現在の実装範囲 |
...
| F10 | UIをWorkerから分離し、Host側の交換可能なSurfaceとして扱う | terminal / Surface I/OはHost、Workerはheadless | **部分実装**。data-only presentation contractとHost-owned TUI adapterはある。Surfaceを選択・load・置換する一般product interfaceはない |
...
| F16 | Worker generationより長く存続するdurable AgentInstanceを持つ | Hostがstable identity、metadata、active Definition bindingを所有する | **未実装**。現`instanceCorrelation`は`WorkerHostSession`ごとに生成され、永続化されない |
...
| C04 | 常にaddress可能なagent service | resident Hostとservice supervisorがlifecycleを所有。Worker常駐は不要 | **条件付き未採用** |
...
| --- | --- | --- | --- |
| Surface / human interaction | F01、F05、F10、F20、F22 | TUI、non-interactive command、presentation contract、session picker | 改訂開始、候補readback、採用の人間向けinterface。Surface一般化は必要になったloopで判断 |
| HenjiHost lifecycle | F07、F11、F14、F16–F18 | WorkerCapsule、WorkerHostSession、startup Definition pre-read、turn correlation | durable Instance、generation replacement、Instance writer admission、revision transition |
...
| optional resident capabilities | C01–C05 | なし | 各機能を採用したloopでのみHost責務とprotocolを具体化 |
...
3. mailbox、routing、schedule、resident service等は、通常利用から必要性が示された場合だけ別loopで
...
継続する人間判断は、F01のintegrated TUIを実際の日常利用で最終採用するかである。通常利用で具体的な
支障が観測された場合だけ、対応する機能incrementを次のproduct loopとして計画する。自己改訂の将来構想
...
作る。mailbox、routing、schedule、resident service、Definition以外の改訂対象は含めない。
...
- process-per-useのHostでこのphaseを成立させるか、resident Hostが必要になるまで延期するか。

このphaseで決めないこと:

- mailbox、routing、schedule、常時稼働service。
- candidateの形式や採用UI。
- Definition以外の改訂対象。
...
- Workerを終了・再生成してもInstanceとrevision bindingが変わらない。
...
| deployment、service supervision、migration | C04を採用するloop | process-per-useでCycle 1が成立する限り延期する |
