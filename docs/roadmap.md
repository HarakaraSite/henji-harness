...
- 段階（実装順序）:
  ...
  9. Increment 70（実装済み）: tool宣言のownerを各Agent Definitionへ統一し、Henji helperがDefinition由来の追加tool
     ...
     他work toolのDefinition化とtool Definition transportは後続へ回す。旧「tool same-identity override」の内容は
     Increment 69へ包含した。
- architectureは
