...
const streamingResponse = (payload: unknown): Response => {
  const id = 'offline-runtime-process-response';
...
const workspaceOption = rawApplicationArgs[0] === '--workspace-root'
  ...
  : undefined;
if (
  rawApplicationArgs[0] === '--workspace-root' && workspaceOption === undefined
) {
...
const toolResultTexts = (body: Record<string, unknown>): string[] => {
  ...
  if (!Array.isArray(messages)) {
    throw new Error('provider request messages invalid');
  }
...
const expectToolResults = (
  ...
  if (
    ...
    actual.some((text, index) => text !== expected[index])
  ) {
    throw new Error(
      'provider request did not contain expected prior tool results',
    );
  }
};
...
const fakeFetch: typeof fetch = (_input, init) => {
  ...
  if (typeof init?.body !== 'string') {
    throw new Error('provider request body missing');
  }
  let body: unknown;
  try {
  ...
  if (fixtureMode === 'argv-delegation-success') {
    ...
    if (requestCount === 1) {
      if (
        JSON.stringify(messages[0]) !==
          JSON.stringify({ role: 'user', content: expectedTask }) ||
    ...
    if (requestCount === 2) {
      ...
      if (
        ...
        serialized.includes('delegate_to_planner') ||
        serialized.includes('parent delegation')
  ...
  if (fixtureMode === 'argv-skill-success') {
    ...
    if (requestCount === 1) {
      ...
      if (
        !serialized.includes('PROCESS-SKILL-BODY') ||
        serialized.includes(workspaceOption ?? '')
      ) {
    ...
    if (
      ...
      !(systemMessages[0] as { content?: unknown }).content?.toString()
        .endsWith(
      ...
      serialized.includes('name":"bash') ||
      serialized.includes('name":"edit') ||
    ...
    for (
      ...
      if (serialized.includes(marker)) {
        throw new Error('instruction marker leaked to provider');
      }
    }
  } else if (
  ...
  if (
    ...
    if (requestCount === 1 && priorResults.length !== 0) {
      throw new Error(
        'first provider request unexpectedly contained tool results',
      );
    }
    if (fixtureMode === 'argv-json-success') {
      if (requestCount === 2) {
        ...
        if (
          ...
          throw new Error(
            'provider request did not contain expected prior work results',
          );
        }
        ...
        if (
          ...
          throw new Error(
            'provider request did not contain expected bash result',
          );
        }
      } else if (requestCount > 5) {
        throw new Error(
          'provider request exceeded successful process sequence',
        );
      }
    } else if (requestCount === 2) {
      if (
        ...
        throw new Error(
          'provider request did not contain expected rejection results',
        );
      }
  ...
  if (fixtureMode === 'argv-skill-success') {
    return requestCount === 1
      ? Promise.resolve(
        workToolResponse('skill', { name: 'process-skill' }, 'skill-1'),
      )
  ...
  if (fixtureMode === 'argv-json-success') {
    ...
    if (requestCount === 2) {
      return Promise.resolve(
        workToolResponse('read', { path: 'process.txt' }, 'read-1'),
      );
    }
    if (requestCount === 3) {
      return Promise.resolve(
    ...
    if (requestCount === 4) {
      return Promise.resolve(
        workToolResponse(
          'bash',
          { command: 'cat process.txt; printf err >&2' },
          'bash-1',
        ),
      );
    }
            ...
            message: {
              ...
              tool_calls: [
                ...
                },
                {
                  ...
                  function: {
                    name: 'read',
                    arguments: JSON.stringify({ path: 'link.txt' }),
                  },
                },
                {
                  id: 'timeout',
...
const stdinIsTerminal = fixtureMode !== 'stdin-success' &&
  fixtureMode !== 'planner-stdin-success';
const workspaceRoot = workspaceOption ??
  await Deno.makeTempDir({ prefix: 'henji-process-work-' });
...
try {
  if (fixtureMode === 'argv-json-failure-recovery') {
    ...
    if (!setup.success) {
      throw new Error('failure recovery workspace setup failed');
    }
  }
  if (fixtureMode === 'argv-filesystem-rejection-success') {
    const setup = await new Deno.Command('/bin/bash', {
    ...
    if (!setup.success) {
      throw new Error('filesystem rejection workspace setup failed');
    }
