...
export const createBashTool = (
  ...
  name: 'bash',
  description:
    "Run one Bash command in a fresh shell starting in the current workspace directory shown in Runtime facts. Shell state does not persist to later bash calls. The parent process environment is not inherited at startup; only PATH, LANG, and LC_ALL are set. If HOME or other variables are needed, specify them within that bash call's command. Default timeout 30000 ms; maximum 120000 ms. stdout and stderr are captured separately. Truncated output can be continued with bash_output.",
